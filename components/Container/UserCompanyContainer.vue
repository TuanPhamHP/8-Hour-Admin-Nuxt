<template>
  <div class="news-container m-0 px-3 py-4 content-slot-container">
    ...
    <DialogConfirm
      ...
      :title="sysLanguage.confirmDialog.removeCompany"
    ...
    />
    <DialogUpdateCompany
      :isVisible="showUpdateData"
      :selected-data="selectedDataToUpdate"
      :cancelAction="cancelUpdateData"
      :confirmAction="addData"
    />
  </div>
</template>
...
import UserCompanyTable from '../Table/UserCompanyTable.vue';
import DialogCreateCompany from '../Dialog/DialogCreateCompany.vue';
import DialogUpdateCompany from '../Dialog/DialogUpdateCompany.vue';
...
export default {
  ...
  computed: {
    ...
    removeMsgComputed() {
      return `Bạn có chắc chắn muốn xóa công ty <b>${
  ...
  methods: {
    ...
    selectDataToUpdate(_data) {
      this.showUpdateData = true;
      this.selectedDataToUpdate = { ..._data };
    },
    viewDetail(id) {
    ...
    async confirmRevoke() {
      ...
      const res = await api.companies.delete(this.selectedDataToRemove.id);
      ...
      try {
        ...
        this.$store.commit(
          'toast/getSuccess',
          this.sysLanguage.snackbar.removeCompanySuccess
        );
    ...
    async getListData() {
      ...
      const query = { ...this.$route.query };
      const payload = { ...query };
      const res = await api.companies.getListData(payload);
      this.isFirstLoading = false;
      ...
      try {
        ...
        this.listData = dataObj.companies;
