  ...
  <div class="user-group-container m-0 px-3 py-4 content-slot-container">
    <div class="content-slot-header">
      ...
      <v-btn
        ...
        @click="showCreateUserGroup = true"
      >
        Tạo nhóm
      </v-btn>
    </div>
    ...
    >
      <v-row class="m-0 align-end filter-wr no-wrap">
        <v-col style="max-width: 400px" cols="auto">
...
import DialogUpdateUserGroup from '../Dialog/DialogUpdateUserGroup.vue';
import DialogCreateUserGroup from '../Dialog/DialogCreateUserGroup.vue';
import UserGroupTable from '../Table/UserGroupTable.vue';
...
export default {
  ...
  methods: {
    async selectDataToUpdate(_data) {
      ...
      const res = await api.userGroup.getDetailUserGroup(id, payload);
      ...
      try {
        ...
        this.selectedDataToUpdate = dataObj.user_group;
    ...
    async getListData() {
      ...
      const query = { ...this.$route.query };
      const payload = { ...query };
      const res = await api.userGroup.getListUserGroup(payload);
      this.isFirstLoading = false;
      ...
      try {
        ...
        const dataObj = res.data.data;
        this.listData = dataObj.user_groups;
